...
from .MODLayer import MoD, DecoderLayer
...
class MTL_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        # else:
        #     self.multi_head_combine = nn.Linear(head_num * qkv_dim, embedding_dim)
        self.layers = nn.ModuleList([DecoderLayer(**model_params) if self.model_params['decoder_layer_num'] % 2 == 0 else
                                     MoD(**model_params)])
