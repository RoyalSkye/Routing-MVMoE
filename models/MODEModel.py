...
class MODEModel(nn.Module):
    ...
    def forward(self, state, selected=None):
        ...
        # first normalize for the embeddings
        # self.encoded_nodes = self.embedding_norm(self.encoded_nodes.permute(0, 2, 1)).permute(0, 2, 1)
...
class MODLEHDModel(nn.Module):
    ...
    def __init__(self, **model_params):
        ...
        self.decoder = MTL_LEHD_Decoder(**model_params)
        self.embedding_norm = nn.InstanceNorm1d(model_params['embedding_dim'])
        self.encoded_nodes = None  # shape: (batch, problem+1, EMBEDDING_DIM)
        self.device = torch.device('cuda', torch.cuda.current_device()) if 'device' not in model_params.keys() else model_params['device']
    ...
    def forward(self, state, selected=None, selected_node_list=None):
        batch_size = state.BATCH_IDX.size(0)
        pomo_size = state.BATCH_IDX.size(1)
        # first normalize for the embeddings
        # self.encoded_nodes = self.embedding_norm(self.encoded_nodes.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        #     # probs = torch.ones(size=(batch_size, pomo_size, self.encoded_nodes.size(1)))
            ...
            attr = torch.cat((state.load[:, :, None], state.current_time[:, :, None], state.length[:, :, None], state.open[:, :, None]), dim=2)
            # shape: (batch, pomo, 4)
            probs, moe_loss = self.decoder(self.encoded_nodes, attr, mask=state.ninf_mask, selected_node_list=selected_node_list)
            self.aux_loss += moe_loss
            # shape: (batch, pomo, problem+1)
            if selected is None:
                while True:
