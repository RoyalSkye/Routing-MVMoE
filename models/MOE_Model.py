...
class MOEModel(nn.Module):
    """
        ...
        4. HELD or LEHD?
        5. Efficient attention, e.g., FlashAttention-2
    """
...
class FF_MOE_Module(nn.Module):
    """
        Implementation of MOE, where each expert is a 2-Layer MLP.
        Routing mechanism: TBA.
    """
    def __init__(self, **model_params):
        super().__init__()
        self.num_experts = model_params['num_experts']
        self.topk = model_params['topk']

        self.experts = nn.ModuleList(
            [
                Feed_Forward_Module(**model_params)
    ...
    def expert_routing(self, input):
        """
            ...
            Cons: Some tokens may not pass any FFN.
                input: (batch_size, problem, embedding)
            return:
                expert_inputs: (num_experts, topk, embedding)
        ...
        G, I = torch.topk(S.T, k=self.topk, dim=-1)
    ...
    def forward(self, input):
        ...
        expert_inputs, expert_weights, expert_index = self.expert_routing(input)  # (num_experts, topk)
        expert_weights, expert_index = expert_weights.unsqueeze(-1), expert_index.flatten()

        # distribute inputs to corresponding expert
        expert_outputs = [self.experts[i](expert_inputs[i]) for i in range(self.num_experts)]  # [(topk, embedding), ...]

        # combine the outputs via weighted sum
        expert_outputs = torch.stack(expert_outputs, 0)  # (num_experts, topk, embedding)
        expert_outputs = expert_outputs.mul(expert_weights)

        # scatter the outputs
        input_ = input.reshape(-1, embedding).scatter_(0, expert_index.unsqueeze(1).repeat(1, embedding), 0)  # (batch x problem, embedding)
        output = input_.index_add_(0, expert_index, expert_outputs.reshape(-1, embedding))
        output = output.reshape(batch_size, problem, embedding)
