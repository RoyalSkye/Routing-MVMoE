...
class VRPBTWEnv:
    ...
    def step(self, selected):
        ...
        #   a. max(current_time + travel_time, tw_start) + service_time <= tw_end
        #   b. vehicle should return to the depot: max(current_time + travel_time, tw_start) + service_time + dist(node, depot)/speed <= self.max_interval
        ...
        fail_return_depot = service_end_time + (self.depot_node_xy[:, None, :1, :] - self.depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)).norm(p=2, dim=-1) / self.speed > self.max_interval + round_error_epsilon
        # shape: (batch, pomo, problem+1)
