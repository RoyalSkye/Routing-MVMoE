...
class OVRPTWEnv:
    ...
    def sample_multiple(self, num_samples):
        ...
        self.step_state.POMO_IDX = torch.arange(self.pomo_size)[None, :].expand(self.batch_size, self.pomo_size).to(
            self.device)
        self.reset_state.current_time = torch.zeros(size=(self.batch_size, self.pomo_size)).to(self.device)
        ...
        self.depot_node_service_time = self.depot_node_service_time.repeat(num_samples, 1)
        self.depot_node_tw_start = self.depot_node_tw_start.repeat(num_samples, 1)
        self.depot_node_tw_end = self.depot_node_tw_end.repeat(num_samples, 1)
